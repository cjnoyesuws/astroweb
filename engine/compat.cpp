...
short comp_aspect_count[15];
short   comp_scout_planet;
short comp_sign_count[12];
short comp_house_count[13];
...
void do_compat_sum_calc( AS_INF *info, AS_INF *partner, unsigned int mode )
{
...
void do_compatab( AS_INF *you, AS_INF *partner, unsigned int mode )
{
 AS_INF *ptr, *ptr2;
 short i;
 short maxp;
 ...
 if ( mode & ASPECTS )
    process_aspects( you, partner, mode & (unsigned int) ( NOBIRTHTIM | VERT_EAST ), COMPA_CH );
...
void do_comp_calc( AS_DATA *ydata, AS_INF *yinf, AS_DATA *pdata, AS_INF *pinf,
	    unsigned int ymode, unsigned int pmode, short house_proc, DATA_PACKET *dpk )
{
 unsigned int mask = ( 0xffff ^ ( BASE_MID | FULL_MID | MID_ASPECT ) );
 short array[13];
 DATA_PACKET xdp;
 unsigned int xymode = ymode, xpmode = pmode;
 ymode &= mask;
 pmode &= mask;
 ...
 if ( errno == ENOMEM )
    return;
 memcpy( array, house_cusps, sizeof(short) * 13 );
 xdp = *dpk;
 xdp.rdp = dpk->crdp;
 do_calc( pinf, pdata, pmode, house_proc, &xdp );
 memcpy( alt_cusps, house_cusps, sizeof(short) * 13 );
 memcpy( house_cusps, array, sizeof(short) * 13 );
 if ( errno == ENOMEM )
    return;
 if ( pmode & SUMMARY )
 ...
 if ( errno == ENOMEM )
    return;
 memcpy( house_cusps, alt_cusps, sizeof(short) * 13 );
