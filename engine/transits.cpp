...
void do_a_transit( AS_INF near *binf, AS_DATA *tdata, AS_INF *tinf,
	       unsigned int mode, short house_mode, DATA_PACKET *dpk, int tno )
{
 AS_INF *ptr, *t;
 short i;
 ...
 memcpy( house_cusps, alt_cusps, 13 * sizeof( short ) );
...
void date_just( AS_DATA *tdata, DATES *newd, short days, short months )
{
...
void manage_transits( AS_INF *binf, AS_INF *tinf, AS_DATA *bdata,
    AS_DATA *tdata, unsigned  int bmode, unsigned int tmode, short house_mode, DATA_PACKET *dp )
{
 short save[13], save2[13];
 short num, x;
 unsigned int mask = ( 0xffff ^ ( BASE_MID | FULL_MID | MID_ASPECT | SUMMARY ) );
 ...
 if ( !(dp->chart_code & COMPO_CH) ) {
    ...
    memcpy( alt_cusps, house_cusps, 13 * sizeof( short ) );
    }
 else {
    memcpy( save, house_cusps, 13 * sizeof( short ) );
    memcpy( save2, alt_cusps, 13 * sizeof( short ) );
    }
 ...
 if ( !(dp->chart_code & COMPO_CH) ) {
    memcpy( save, house_cusps, 13 * sizeof( short ) );
    memcpy( house_cusps, alt_cusps, 13 * sizeof( short ) );
    memcpy( alt_cusps, save, 13 * sizeof( short ) );
    }
 else {
    memcpy( house_cusps, save, 13 * sizeof( short ) );
    memcpy( alt_cusps, save2, 13 * sizeof( short ) );
    }
